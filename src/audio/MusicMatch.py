...
class MusixMatchAPI:
    ...
    def get_track_subtitle(
        self,
        commontrack_id: str = None,
        track_id: str = None,
        track_isrc: str = None,
        subtitle_format: str = "lrc",
        f_subtitle_length: str = None,
        f_subtitle_length_max_deviation: str = None,
    ) -> dict:
        """
        ...
        Args:
            ...
            subtitle_format (str): The format of the subtitle (lrc, dfxp, mxm). Default to 'lrc', use 'mxm' for getting the subtitle as a json string.
            f_subtitle_length (str): The desired length of the subtitle (seconds).
            f_subtitle_length_max_deviation (str): The maximum deviation allowed from the f_subtitle_length (seconds).

        Returns:
            dict: The API response containing subtitle data.
        """
        if not (commontrack_id or track_id or track_isrc):
            raise ValueError("At least one of commontrack_id, track_id, or track_isrc must be provided.")

        base_url = f"{EndPoints.GET_TRACK_SUBTITLE.value}?app_id=web-desktop-app-v1.0&format=json"
        ...
        if subtitle_format:
            base_url += f"&subtitle_format={subtitle_format}"
        if f_subtitle_length:
            base_url += f"&f_subtitle_length={f_subtitle_length}"
        if f_subtitle_length_max_deviation:
            base_url += f"&f_subtitle_length_max_deviation={f_subtitle_length_max_deviation}"
